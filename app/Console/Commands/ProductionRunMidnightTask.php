...
class ProductionRunMidnightTask extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:production-run-midnight-task {task}';
    ...
    public function handle()
    {
        ...
        try {
            switch ($task) {
                case 'production_transfer_data_task':
                    $this->production_transfer_data_task();
                    break;
                default:
                    $this->error('Unknown task. Available tasks: 🚀 production_transfer_data_task');
                    break;
            }
        } catch (\Throwable $th) {
            $this->error($th);
    ...
    {
        $now = now();
        $start = $now->copy()->setTime(10, 30); // 18:30
