...
@section('content')
                ...
                <div class="pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-clip-padding text-current shadow-4 outline-none bg-gray-100 dark:bg-[#202020]">
                    ...
                    <form id="form_product_group" class="" method="POST">
                        ...
                        <div class="flex flex-shrink-0 flex-wrap items-center justify-end rounded-b-md p-2">
                            <button data-twe-modal-dismiss id="submitButton" type="button" class="text-white bg-[#303030] hover:bg-[#404040] font-bold py-2 px-4 rounded group cursor-not-allowed opacity-50" onclick="updateOrCreateProductType()" disabled>
    ...
    <script>
        ...
        function updateOrCreateProductType() {
            jQuery.ajaxSetup({
                headers: {
            ...
            if(Edit_ProductType_ID) {
                url = "{{ route('product_detail.product_type_update', 0) }}".replaceAll('/0', '/' + Edit_ProductType_ID);
            } else {
                url = "{{ route('product_detail.product_type_create') }}"
            }
