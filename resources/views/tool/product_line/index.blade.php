...
@section('content')
    <div class="justify-center items-center">
        <div class="mt-6 mb-4 flex justify-center items-center">
            <p class="inline-block space-y-2 border-b-2 border-gray-200 dark:border-gray-700 text-xl font-bold text-gray-900 dark:text-gray-100">Product Line</p>
        </div>
            ...
            <div data-twe-modal-dialog-ref class="pointer-events-none relative w-auto translate-y-[-50px] opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:max-w-[500px]">
                <div class="pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-clip-padding text-current shadow-4 outline-none bg-gray-100 dark:bg-[#202020]">
                    ...
                    <form id="form_product_group" class="" method="POST">
                        <input class="" type="hidden" id="Edit_ProductLine_ID" name="ID" value="">
                        ...
                        <div class="p-4 text-gray-900 dark:text-gray-100" style="position: relative;">
                            ...
                            <input type="text" id="ID" name="ID" onkeyup="checkProductLineId()" class="h-10 border-[#303030] dark:border focus:border-blue-500 rounded-sm px-4 w-full bg-gray-50 dark:bg-[#303030] text-center" value="" />
                        ...
                        <div class="p-4 text-gray-900 dark:text-gray-100" style="position: relative;">
                            ...
                            <input type="text" id="DESCRIPTION" name="DESCRIPTION" onkeyup="checkProductLineName()" class="h-10 border-[#303030] dark:border focus:border-blue-500 rounded-sm px-4 w-full bg-gray-50 dark:bg-[#303030] text-center" value="" />
                            <div class="col-auto" style="position: absolute; right: 2.5%; top: 57.2%;">
                ...
                <table id="table_product_line" class="table table-striped table-bordered dt-responsive nowrap text-gray-900 dark:text-gray-100" style="width:100%">
                    <thead>
                        ...
                        <tr>
                            ...
                            <th>
                                <select class="js-example-basic-single w-full rounded-sm text-xs" id="searchProductLineId" name="" onchange="productLineSearch()">
                            ...
                            <th>
                                <input type="text" name="" id="searchProductLineName" class="h-10 border-[#303030] dark:border focus:border-blue-500 mt-1 rounded-sm px-4 w-full bg-gray-50 dark:bg-[#303030] text-center" placeholder="ประเภทสินค้า . . ." value="" onkeyup="searchTable()" />
                            </th>
                            <th>
                            </th>
                            <th>
    ...
    <script>
        ...
        function checkProductLineId() {
            const Edit_ProductLine_ID = jQuery('#Edit_ProductLine_ID').val();
            const ID = jQuery('#ID').val();

            jQuery.ajax({
                method: "POST",
                url: "{{ route('product_detail.product_line_check_id') }}",
                data: {
                        _token: "{{ csrf_token() }}",
                        Edit_ProductLine_ID, ID
                    },
                dataType: 'json',
                beforeSend: function () {
                    isValidID = false;
        ...
        function checkProductLineName() {
            ...
            jQuery.ajax({
                method: "POST",
                url: "{{ route('product_detail.product_line_check_name') }}",
                data: {
                        _token: "{{ csrf_token() }}",
                        Edit_ProductLine_ID, DESCRIPTION
                    },
                dataType: 'json',
                beforeSend: function () {
        ...
        jQuery("#ID").on("keyup", checkProductLineId);
        jQuery("#DESCRIPTION").on("keyup", checkProductLineName);

        function modelProductLine(ID, CATEGORY_ID, DESCRIPTION) {
            console.log("🚀 ~ modelProductLine ~ id:", ID, CATEGORY_ID, DESCRIPTION)
            if (ID) {
                ...
                jQuery("#staticBackdropLabel").text('แก้ไข Product Line');
                ...
                jQuery("#Edit_ProductLine_ID").val('')
                ...
                jQuery("#staticBackdropLabel").text('เพิ่ม Product Line');
        ...
        const mytableDatatable = $('#table_product_line').DataTable({
            ...
            "ajax": {
                ...
                "url": "{{ route('product_detail.list_product_line') }}",
                "type": "POST",
                'data': function(data) {
                    // Read values
                    data.searchProductLineId = $('#searchProductLineId').val();
                    data.searchProductLineName = $('#searchProductLineName').val();
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<div class="inline-flex flex items-center rounded-md shadow-sm">
                                    <button
                                        ...
                                        data-twe-ripple-init
                                        data-twe-ripple-color="light"
                                        onclick="modelProductLine('${row.ID}', '${row.CATEGORY_ID}','${row.DESCRIPTION}')"
                                        type="button"
