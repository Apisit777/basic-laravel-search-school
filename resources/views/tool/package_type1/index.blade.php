...
@section('content')
    <div class="justify-center items-center">
        <div class="mt-6 mb-4 flex justify-center items-center">
            <p class="inline-block space-y-2 border-b-2 border-gray-200 dark:border-gray-700 text-xl font-bold text-gray-900 dark:text-gray-100">Package Type1</p>
        </div>
            ...
            <div data-twe-modal-dialog-ref class="pointer-events-none relative w-auto translate-y-[-50px] opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:max-w-[500px]">
                <div class="pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-clip-padding text-current shadow-4 outline-none bg-gray-100 dark:bg-[#202020]">
                    ...
                    <form id="form_product_group" class="" method="POST">
                        <input class="" type="hidden" id="Edit_PackageType1_ID" name="ID" value="">
                        <div class="p-4 text-gray-900 dark:text-gray-100" style="position: relative;">
                            <label for="ID">ID</label>
                            <input type="text" id="ID" name="ID" onkeyup="checkPackageType1Id()" class="h-10 border-[#303030] dark:border focus:border-blue-500 rounded-sm px-4 w-full bg-gray-50 dark:bg-[#303030] text-center" value="" />
                        ...
                        <div class="p-4 text-gray-900 dark:text-gray-100" style="position: relative;">
                            ...
                            <input type="text" id="DESCRIPTION" name="DESCRIPTION" onkeyup="checkPackageType1Name()" class="h-10 border-[#303030] dark:border focus:border-blue-500 rounded-sm px-4 w-full bg-gray-50 dark:bg-[#303030] text-center" value="" />
                            <div class="col-auto" style="position: absolute; right: 2.5%; top: 57.2%;">
            ...
            <div id="containerexample" class="text-gray-900 dark:text-gray-100">
                <table id="table_usage_area" class="table table-striped table-bordered dt-responsive nowrap text-gray-900 dark:text-gray-100" style="width:100%">
                    <thead>
                        ...
                        <tr>
                            <th>
                                <select class="js-example-basic-single w-full rounded-sm text-xs" id="searchPackageType1Id" name="" onchange="packageType1Search()">
                                    <option value="" class="text-xs"> --- กรุณาเลือก ---</option>
                                    @foreach ($packageType1s as $key => $packageType1)
                                        <option value="{{ $packageType1 }}">{{ $packageType1 }}</option>
                                    @endforeach
                                </select>
                            </th>
                            <th>
                            <input type="text" name="" id="searchPackageType1Name" class="h-10 border-[#303030] dark:border focus:border-blue-500 mt-1 rounded-sm px-4 w-full bg-gray-50 dark:bg-[#303030] text-center" placeholder="ประเภทสินค้า . . ." value="" onkeyup="searchTable()" />
                            </th>
                            <th>
                            </th>
                            <th>
    ...
    <script>
        ...
        function checkPackageType1Id() {
            const Edit_PackageType1_ID = jQuery('#Edit_PackageType1_ID').val();
            const ID = jQuery('#ID').val();

            jQuery.ajax({
                method: "POST",
                url: "{{ route('product_detail.package_type1_check_id') }}",
                data: {
                        _token: "{{ csrf_token() }}",
                        Edit_PackageType1_ID, ID
                    },
                dataType: 'json',
                beforeSend: function () {
                    isValidID = false;
        ...
        function checkPackageType1Name() {
            const Edit_PackageType1_ID = jQuery('#Edit_PackageType1_ID').val();
            const DESCRIPTION = jQuery('#DESCRIPTION').val();

            jQuery.ajax({
                method: "POST",
                url: "{{ route('product_detail.package_type1_check_name') }}",
                data: {
                        _token: "{{ csrf_token() }}",
                        Edit_PackageType1_ID, DESCRIPTION
                    },
                dataType: 'json',
                beforeSend: function () {
        ...
        jQuery("#ID").on("keyup", checkPackageType1Id);
        jQuery("#DESCRIPTION").on("keyup", checkPackageType1Name);

        function modelProductGroup(ID, DESCRIPTION) {
            console.log("🚀 ~ modelProductGroup ~ ID:", ID, DESCRIPTION)
            if (ID) {
                jQuery("#Edit_PackageType1_ID").val(ID)
                ...
                jQuery("#staticBackdropLabel").text('แก้ไข Package Type1')
                ...
                jQuery("#Edit_PackageType1_ID").val('')
                ...
                jQuery("#staticBackdropLabel").text('เพิ่ม Package Type1')
        ...
        const mytableDatatable = $('#table_usage_area').DataTable({
            ...
            "ajax": {
                ...
                "url": "{{ route('product_detail.list_package_type1') }}",
                "type": "POST",
                'data': function(data) {
                    // Read values
                    data.searchPackageType1Id = $('#searchPackageType1Id').val();
                    data.searchPackageType1Name = $('#searchPackageType1Name').val();
        ...
        function createProductGroup() {
            ...
            if(Edit_PackageType1_ID) {
                url = "{{ route('product_detail.package_type1_update', 0) }}".replaceAll('/0', '/' + Edit_PackageType1_ID);
            } else {
                url = "{{ route('product_detail.package_type1_create') }}"
            }
