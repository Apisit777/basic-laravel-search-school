...
@section('content')
            ...
            <div data-twe-modal-dialog-ref class="pointer-events-none relative w-auto translate-y-[-50px] opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:max-w-[500px]">
                <div class="pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-clip-padding text-current shadow-4 outline-none bg-gray-100 dark:bg-[#202020]">
                    ...
                    <form id="form_product_group" class="" method="POST">
                        <input class="" type="hidden" id="Edit_Series_ID" name="ID" value="">
                        <div class="p-4 text-gray-900 dark:text-gray-100" style="position: relative;">
                            <label for="ID">ID</label>
                            <input type="text" id="ID" name="ID" onkeyup="checkSeriesId()" class="h-10 border-[#303030] dark:border focus:border-blue-500 rounded-sm px-4 w-full bg-gray-50 dark:bg-[#303030] text-center" value="" />
                        ...
                        <div class="p-4 text-gray-900 dark:text-gray-100" style="position: relative;">
                            ...
                            <input type="text" id="DESCRIPTION" name="DESCRIPTION" onkeyup="checkSeriesName()" class="h-10 border-[#303030] dark:border focus:border-blue-500 rounded-sm px-4 w-full bg-gray-50 dark:bg-[#303030] text-center" value="" />
                            <div class="col-auto" style="position: absolute; right: 2.5%; top: 57.2%;">
    ...
    <script>
        ...
        function checkSeriesId() {
            const Edit_Series_ID = jQuery('#Edit_Series_ID').val();
        ...
        jQuery("#ID").on("keyup", checkSeriesId);
        jQuery("#DESCRIPTION").on("keyup", checkSeriesName);

        function modelProductGroup(id, DESCRIPTION) {
            console.log("🚀 ~ modelProductGroup ~ id:", id, DESCRIPTION)
            if (id) {
                jQuery("#Edit_Series_ID").val(id)
        ...
        const mytableDatatable = $('#product_group').DataTable({
            ...
            "ajax": {
                ...
                "url": "{{ route('product_master.list_series') }}",
                "type": "POST",
                'data': function(data) {
                    // Read values
        ...
        function createProductGroup() {
            ...
            if(Edit_Series_ID) {
                url = "{{ route('product_master.series_update', 0) }}".replaceAll('/0', '/' + Edit_Series_ID);
            } else {
