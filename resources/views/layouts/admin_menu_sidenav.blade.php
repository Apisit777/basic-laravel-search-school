    ...
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-[#202020] duration-500 mt">
        <!-- @foreach($routeName as $menu)
            <ul class="space-y-2 font-medium">
                    ...
                    <li class="relative w-[232px] overflow-hidden group mt-2">
                        ...
                        @foreach($menu['getPermissionSubmenus'] as $submenu)
                            <ul class="bg-[#f9f9f9] dark:bg-[#232323] overflow-hidden transition-all duration-500 max-h-0 peer-checked:max-h-40 h-fit after:absolute after:left-[1.20rem] after:top-[2.5rem] after:mt-px after:h-[calc(100%-2.45rem)] after:w-px after:bg-neutral-200 after:content-[''] dark:after:bg-gray-700">
                                <li>
                                    <a href="#" class="flex items-center w-full p-1.5 text-gray-900 transition duration-75 rounded-sm pl-12 group hover:bg-gray-100 dark:text-white dark:hover:bg-[#303030] {{ Request::is($submenu->url.'*') ? 'rounded-sm bg-primary-100 dark:bg-[#014a77] duration-500': '' }}">
                                        {{ $submenu->name }}
                                    </a>
                                </li>
                            </ul>
                ...
                @else
                    <li class="mt-2">
                        <a class="flex items-center p-2 text-gray-900 rounded-sm dark:text-white hover:bg-gray-100 dark:hover:bg-[#303030] group {{ Request::is($menu['url'].'*') ? 'rounded-sm bg-primary-100 dark:bg-[#014a77] duration-500': '' }}" href="{{ '/'.$menu['url'] }}">
