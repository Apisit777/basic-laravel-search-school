...
<body>
    ...
    <div class="min-h-screen p-10"
        style="background-image: url('https://www.ssup.co.th/wp-content/uploads/2022/11/shutterstock_2079577573.png')">
        <div class="g-2 flex flex-wrap items-center justify-center lg:justify-between">
            ...
            <div class="mb-12 md:mb-0 md:w-8/12 lg:w-5/12 xl:w-4/12">
                <div
                    class="relative max-w-7xl rounded-md p-10 bg-white dark:bg-[#202020] text-black dark:text-white shadow-md shadow-[#202020] dark:shadow-blue-500 mt-24">
                    <form id="from_user" class="group js-validation-signin" action="javascript:void(0)" method="POST">
                        ...
                        <div
                            class="my-4 flex items-center before:mt-0.5 before:flex-1 before:border-t before:border-black dark:before:border-blue-500 after:mt-0.5 after:flex-1 after:border-t after:border-black dark:after:border-blue-500">
                        ...
                        <div class="flex flex-col">
                            <div class="flex w-full flex-col items-start">
                                <label for=""
                                    class="block text-sm font-normal text-black dark:text-white">Username</label>
                                ...
                                <span
                                    class="mt-2 hidden text-sm text-red-500 peer-[&:not(:placeholder-shown):not(:focus):invalid]:block">Please
                            ...
                            <div class="mt-2">
                                <label for=""
                                    class="block text-sm font-normal text-black dark:text-white">Username</label>
                                <select
                                    class="js-example-basic-single w-full bg-white dark:bg-[#2020] rounded-sm text-xs switch_role"
                                    name="name_position">
                                    <option value=""> --- กรุณาเลือก ---</option>
                                    @foreach (Auth::user()->userRole->pluck('position.name_position') as $name_position)
                                        <!-- <option value="{{ $name_position }}" {{ $name_position == Auth::user()->userRole->first()->position->name_position ? 'selected' : '' }}>{{ $name_position }}</option> -->
                                        <option value="{{ $name_position }}">{{ $name_position }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="flex w-full mt-8">
                            <button 
                                onClick="checkLogin()"
                                type="button"
                                class="w-full cursor-pointer rounded bg-primary-600 px-5 py-0.5 text-center text-sm font-medium text-white hover:bg-primary-700 focus:outline-none focus:ring-4 focus:ring-blue-300 group-invalid:pointer-events-none group-invalid:opacity-60">
                        ...
                        <div id="loading_form" class="loading_ absolute !hidden bg-[#e4e4e4e3] dark:bg-[#2e2d2dd5]">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                class="w-10 h-10 animate-spin dark:text-white">
                                <path
                    ...
                    <div class="flex w-full mt-3">
                        <a href="{{ route('product_master.index') }}" type="button"
                            class="w-full cursor-pointer rounded text-center text-sm font-medium text-gray-100 bg-[#303030] hover:bg-[#404040] py-1.5 px-4 group">
                            <svg fill="#fff"
                                class="-mt-1 size-5 hidden h-6 w-6 transition-transform duration-300 group-hover:translate-x-1 rtl:rotate-180 rtl:group-hover:-translate-x-1 md:inline-block"
                                version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                                xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 26.676 26.676"
                                xml:space="preserve">
                                <g>
...
<script>
    ...
    function checkLogin() {
        ...
        let dataRole = JSON.parse(dataLogin)
        console.log("🚀 ~ checkLogin ~ dataRole:", dataRole.data)
        dataForm.append('dataRole', JSON.stringify(dataRole.data));
        ...
        jQuery.ajax({
            ...
            url: "/role_bypass",
            data: dataForm,
            // data: { dataForm: dataForm, dataRole: dataRole.data },
            processData: false,
            contentType: false,
            beforeSend: function () {
            ...
            success: (res) => {
                ...
                sessionStorage.setItem("role", $('.switch_role').val())
    ...
    $(document).ready(function () {
        ...
        $('.switch_role').val(role).trigger('change');
        console.log("🚀 ~ $('.switch_role').val(role).trigger('change');:", $('.switch_role').val())
